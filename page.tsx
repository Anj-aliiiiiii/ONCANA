...
export default function SurveyPage() {
  ...
  const handleNext = async () => {
    if (currentStep < totalSteps) {
      ...
      try {
        // ✅ Get auth data again to ensure fresh value
        const authData = getAuthData();
        const userId = authData?.id;
  
        console.log("📛 Auth data:", authData); // ✅ DEBUG 1
        console.log("📛 Extracted userId:", userId); // ✅ DEBUG 2
        ...
        console.log("📤 Final survey submission payload:", flatSurveyData); // ✅ DEBUG 3
        ...
        if (!response.ok) {
          ...
          console.error("❌ Backend error details:", errorResponse); // ✅ DEBUG 4
        ...
        console.error('🔥 Error submitting survey:', error); // ✅ DEBUG 5
  ...
  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
    ...
    case 7:
      return (
        <div className="space-y-6">
          <div className="space-y-2">
            <Label>Care Coordination Satisfaction (1–5)</Label>
            <Input
              type="number"
              min={1}
              max={5}
          ...
          <div className="space-y-2">
            <Label>Fear of Recurrence Score (0–10)</Label>
            <Input
              type="number"
              min={0}
              max={10}
